...
# 创建NPC工厂函数
def create_npc(name, x, y, dialogue, has_shop=False, map_type='村庄', npc_type='普通'):
    """创建NPC的工厂函数"""
    if map_type == '村庄':
        return VillageNPC(name, x, y, dialogue, has_shop, npc_type)
    elif map_type == '森林':
        return ForestNPC(name, x, y, dialogue, has_shop, npc_type)
    elif map_type == '沙漠':
        return DesertNPC(name, x, y, dialogue, has_shop, npc_type)
    elif map_type == '地牢':
        return DungeonNPC(name, x, y, dialogue, has_shop, npc_type)
    else:
        return BaseNPC(name, x, y, dialogue, has_shop, map_type, npc_type)
